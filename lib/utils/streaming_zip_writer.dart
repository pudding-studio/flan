...
/// Memory-efficient ZIP writer that streams entries directly to disk.
/// Only one file's data is in memory at a time.
class StreamingZipWriter {
  final RandomAccessFile _raf;
  final List<_ZipCentralEntry> _entries = [];

  StreamingZipWriter._(this._raf);

  static Future<StreamingZipWriter> open(String path) async {
    final raf = await File(path).open(mode: FileMode.write);
    return StreamingZipWriter._(raf);
  }
  ...
  Future<void> addFile(String name, List<int> bytes) async {
    ...
    header.setUint16(8, 0, Endian.little);              // STORE
  ...
  Future<void> close() async {
    final cdOffset = await _raf.position();
    int cdSize = 0;
    ...
    for (final entry in _entries) {
      ...
      cd.setUint16(10, 0, Endian.little);
      cd.setUint16(12, 0, Endian.little);
      cd.setUint16(14, 0, Endian.little);
      cd.setUint32(16, entry.crc, Endian.little);
      cd.setUint32(20, entry.size, Endian.little);
      cd.setUint32(24, entry.size, Endian.little);
      cd.setUint16(28, entry.nameBytes.length, Endian.little);
      ...
      cd.setUint32(42, entry.localHeaderOffset, Endian.little);

      await _raf.writeFrom(cd.buffer.asUint8List());
      await _raf.writeFrom(entry.nameBytes);
    ...
    eocd.setUint16(8, _entries.length, Endian.little);
    eocd.setUint16(10, _entries.length, Endian.little);
